...
class VehicleBrandService extends BaseService implements VehicleBrandServiceInterface
{
    ...
    public function create(array $data): ?Model
    {
        $storeData = [
            ...
            'image' => fileUploader('vehicle/brand/', APPLICATION_IMAGE_FORMAT, $data['brand_logo']),
        ];
    ...
    public function update(int|string $id, array $data = []): ?Model
    {
        ...
        if (array_key_exists('brand_logo', $data)) {
            $updateData = array_merge($updateData, [
                'image' => fileUploader('vehicle/brand/', APPLICATION_IMAGE_FORMAT, $data['brand_logo'], $model?->image)
            ]);
        }
