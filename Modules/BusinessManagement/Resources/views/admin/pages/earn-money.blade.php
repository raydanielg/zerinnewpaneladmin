...
@section('title', translate('Earn Money'))
...
@section('content')
    ...
    @php($appVersionControlForAndroid = businessConfig(key: DRIVER_APP_VERSION_CONTROL_FOR_ANDROID, settingsType: APP_VERSION)?->value)
    @php($appVersionControlForIos = businessConfig(key: DRIVER_APP_VERSION_CONTROL_FOR_IOS, settingsType: APP_VERSION)?->value)
    <!-- Main Content -->
        ...
        <div class="container-fluid">
            ...
            <div class="card card-body mb-3">
                <div class="d-flex flex-md-nowrap flex-wrap align-items-center justify-content-between gap-3">
                    <div class="w-0 flex-grow-1">
                        <h4 class="text-capitalize mb-1">{{ translate('Show Earn Money') }}</h4>
                        <div class="fs-14">
                            {{ translate('Allow this option to display the Earn Money Section on the landing page.') }}
                        </div>
                    </div>
                    <label
                        class="max-w300 w-100 form-control rounded d-flex align-items-center justify-content-between">
                        <label for="showEarnMoney"
                               class="fs-14 lh-1 d-block cursor-pointer text-dark">{{ translate('Status') }}</label>
                        <label class="switcher cmn_focus rounded-pill">
                            <input class="switcher_input update-business-setting" id="showEarnMoney"
                                   name="is_earn_money_enabled" type="checkbox" data-name="is_earn_money_enabled"
                                   data-type="{{ EARN_MONEY }}" tabindex="4"
                                   data-url="{{ route('admin.business.pages-media.landing-page.update-landing-page-setting') }}"
                                   data-icon="{{ $isEarnMoneyEnabled == 0 ? dynamicAsset('public/assets/admin-module/img/svg/info-circle-green.svg') : dynamicAsset('public/assets/admin-module/img/svg/info-circle-red.svg') }}"
                                   data-title="{{ $isEarnMoneyEnabled == 0 ? translate('Want to enable Earn Money') : translate('Want to disable Earn Money') }}?"
                                   data-sub-title="{{ $isEarnMoneyEnabled == 0 ? translate(' If you turn on the Earn Money, users will see it in the landing page.') : translate('If you turn off the Earn Money, users will no longer see it in the landing page.') }}"
                                {{ $isEarnMoneyEnabled == 1 ? 'checked' : '' }}
                            >
            ...
            <div class="card card-body mb-3">
                <div class="d-flex justify-content-between align-items-center gap-3 flex-wrap">
                    <div>
                        <h4 class="mb-1">
                            {{ translate('Earn Money Section') }}
                        </h4>
                        <p class="fs-12 mb-0">{{ translate('See how your earn money Section will look to customers.') }}</p>
                    </div>
            ...
            <form action="{{ route('admin.business.pages-media.landing-page.earn-money.update-section') }}"
                ...
                <div class="card card-body mb-3">
                    <div class="mb-20">
                        <h5 class="mb-1">{{ translate('Driver App Download Section Content ') }}</h5>
                    ...
                    <div class="row g-3">
                        <div class="col-lg-8">
                            <div class="p-lg-4 p-3 rounded bg-F6F6F6">
                                <div class="mb-3">
                                    <label for="" class="mb-2">{{ translate('title') }} <span
                                ...
                                <div>
                                    ...
                                    <div class="character-count d-flex flex-column align-items-end">
                                        <input type="text" class="form-control character-count-field" maxlength="200"
                                               ...
                                               value="{{$introContents?->value['subtitle'] ?? ''}}" name="subtitle"
                                               placeholder="{{ translate('Ex: Sub_Title') }}" required tabindex="3">
                        ...
                        <div class="col-lg-4">
                            <div
                                ...
                                <div class="text-center">
                                    ...
                                    <p class="fs-12 mb-0">{{ translate('Upload driver app download Section Image') }}</p>
                                </div>
                                ...
                                <div class="upload-file-new">
                                    <input type="file" class="upload-file-new__input single_file_input" tabindex="4"
                                           ...
                                           {{ $introContents?->value['image'] ? '' : 'required'}} data-max-upload-size="{{ $maxSize }}">
                                    <label class="upload-file-new__wrapper ratio-1-1">
                                        ...
                                        <img class="upload-file-new-img" loading="lazy"
                                             src="{{ $introContents?->value['image'] ? dynamicStorage('storage/app/public/business/landing-pages/earn-money/'.$introContents?->value['image']): '' }}"
                                             data-default-src="{{ $introContents?->value['image'] ? dynamicStorage('storage/app/public/business/landing-pages/earn-money/'.$introContents?->value['image']): '' }}"
                                             alt="">
                                    </label>
                                    <div class="overlay">
                    ...
                    <div class="d-flex justify-content-end gap-3 mt-4">
                        <button class="btn btn-secondary cmn_focus min-w-120" tabindex="5" type="reset">
                            {{ translate('reset') }}
                        </button>
                        <button class="btn btn-primary cmn_focus min-w-120" tabindex="6" type="submit">
                            {{ translate('save') }}
                        </button>
                    </div>
            ...
            <form action="{{ route('admin.business.pages-media.landing-page.earn-money.update-section') }}" id=""
                  enctype="multipart/form-data" method="POST">
                @csrf
                <input type="hidden" name="key_name" value="{{ BUTTON_CONTENTS }}">
                <div class="card card-body mb-3">
                    <div class="mb-20">
                        <h5 class="mb-1">{{ translate('Driver App Download Section Button ') }}</h5>
                        <p class="fs-12 mb-0">{{ translate('Set a title and subtitle that will appear on the landing page to inform about driver app downloads.') }}</p>
                    </div>
        ...
        <form action="javascript:" class="d-flex flex-column h-100">
            <div class="offcanvas-header">
                <h4 class="offcanvas-title flex-grow-1">
                    {{ translate('Earn Money Section Preview') }}
                </h4>
            ...
            <div class="offcanvas-body scrollbar-thin">
                <div class="h-100 overflow-y-auto py-5 px-0">
                    <img src="{{ dynamicAsset('public/assets/admin-module/img/preview/earn-money.png') }}" alt=""
    ...
    @include('businessmanagement::admin.pages.partials._note-modal',['page' => 'earn_money'])
@endsection
