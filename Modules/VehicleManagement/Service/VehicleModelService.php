...
class VehicleModelService extends BaseService implements VehicleModelServiceInterface
{
    ...
    public function create(array $data): ?Model
    {
        $storeData = [
            ...
            'image' => fileUploader('vehicle/model/', APPLICATION_IMAGE_FORMAT, $data['model_image']),
        ];
    ...
    public function update(int|string $id, array $data = []): ?Model
    {
        ...
        if (array_key_exists('model_image', $data)) {
            $updateData = array_merge($updateData, [
                'image' => fileUploader('vehicle/model/', APPLICATION_IMAGE_FORMAT, $data['model_image'], $model?->image)
            ]);
        }
