...
class ParcelCategoryService extends BaseService implements ParcelCategoryServiceInterface
{
    ...
    public function create(array $data): ?Model
    {
        $storeData = [
            ...
            'image'=>fileUploader('parcel/category/', APPLICATION_IMAGE_FORMAT, $data['category_icon']),
        ];
    ...
    public function update(string|int $id, array $data = []): ?Model
    {
        ...
        if (array_key_exists('category_icon', $data)) {
            $updateData = array_merge($updateData,[
                'image' => fileUploader('parcel/category/', APPLICATION_IMAGE_FORMAT, $data['category_icon'], $model?->image)
            ]);
        }
