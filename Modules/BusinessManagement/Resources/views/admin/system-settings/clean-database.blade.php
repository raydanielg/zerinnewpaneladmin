                        ...
                        <div class="row mb-4">
                            <div class="col-sm-6 col-xl-3">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <label class="text-dark text-capitalize custom-checkbox"
                                           for="select-all-modules">{{translate('select_all')}}</label>
                                </div>
                            </div>
                        </div>

                        <form action="{{ route('admin.business.clean-database.clean') }}" method="post"
                                            ...
                                            <div class="card-body d-flex align-items-center justify-content-between">
                                                <div class="d-flex align-items-center gap-2">
                                                    <input type="checkbox" name="tables[]" value="{{ $table }}"
                                                           class="select_all module-checkbox"
                                                           id="table_{{ $key }}">
                                                    <label class="text-dark text-lowercase custom-checkbox"
                                                           for="table_{{ $key }}">{{ $table }}</label>
                                                </div>
                                                <span
                                                    class="badge badge-info">{{ \Illuminate\Support\Facades\DB::table($table)->count() }}</span>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                            ...
                            <div class="d-flex justify-content-end gap-10 flex-wrap mt-3">
                                <button type="{{ env('APP_MODE') != 'demo' ? 'submit' : 'button' }}"
                                        ...
                                        class="btn btn-primary cmn_focus">{{ translate('clear') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
...
@push('script')
    <script
        src="{{ dynamicAsset('public/assets/admin-module/js/business-management/system-settings/clean-database.js') }}"></script>
